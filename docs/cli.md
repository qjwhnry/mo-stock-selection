...
mo-stock run-once --date 2026-04-22 --skip-ai          # 跳过 AI 阶段（v2.2，行为等同 v2.1）
...
- `--skip-ai`（**v2.2 新增**）：跳过 combine 的 AI 分析阶段，行为等同 v2.1 纯规则模式。Anthropic API 故障 / 调试 / 控制成本时使用
- `--strategy short|swing`：策略选择（默认 `short`）。`short`：6 维度短线选股；`swing`：7 维度波段选股 + market_regime 控制
...
流程：规则层 5 维 → TOP 50 候选 → Claude AI 分析 → final_score = rule × 0.6 + ai × 0.4 → 重排 TOP 20 → 报告。
...
行为完全等同 v2.1 纯规则模式。Anthropic API 故障时立即降级使用。

**报告"选出原因"**（自动生成，无需配置）：
- markdown：每只入选股展示 AI 论点 + 关键信号 + 选出维度证据表（5 维 detail 翻译为中文）+ 操作建议（入场/止损）+ 风险
