...
| `exhaustion` 短线透支 | 0.10 | `daily_kline`（近 11 日 OHLCV） | ✓ [ExhaustionFilter](../src/mo_stock/filters/short/exhaustion_filter.py) |

每个维度独立打分，0-100 分。
**只对该维度有正向信号的股 append 结果**（score=0 视为信号缺失，由综合分公式按 0 处理）。
`exhaustion` 例外：作为风险修饰维度，对所有候选股打分（需至少命中其它 5 维之一）。
...
> ⚠️ **关键设计**：连续打分替代旧版离散分档，提升同板块内个股区分度。
...
## 7. exhaustion 维度（短线透支检测）

**目标**：识别短期过度透支的股票，作为风险修饰维度惩罚过热股（不是入场信号）。

**数据源**：`daily_kline`（近 11 个交易日 OHLCV 数据）。

**设计原则**：exhaustion 是唯一的风险修饰维度——候选股需至少命中 limit/moneyflow/lhb/sector/theme 之一，
然后 exhaustion 对这些候选股评估短期透支程度。得分越高表示透支越轻（越安全），得分越低表示透支越严重。
...
| 5 日涨幅 | ≥ 30% | -20 |
| 5 日涨幅 | 20%-30% | -10 |
| MA5 偏离度 | 收盘价 > MA5 × 1.05 | -15 |
| 量价背离 | 5 日内价涨 ≥ 10% 且量缩 ≥ 20% | -15 |
| 连涨天数 | ≥ 5 天 | -15 |
| 连涨天数 | 4 天 | -8 |
| 动量衰减 | 近 3 日涨幅逐日递减 | -10 |

代码：[filters/short/exhaustion_filter.py](../src/mo_stock/filters/short/exhaustion_filter.py)
配置：`weights.yaml: exhaustion_filter`
...
| **exhaustion** | 5 日涨幅低、无透支信号 → 新鲜度 ≈95 | +0 | **95** |

### 综合分

```
final = (0×0.25 + 18×0.25 + 55×0.20 + 45×0.10 + 0×0.10 + 95×0.10) / 1.00
      = 0 + 4.5 + 11 + 4.5 + 0 + 9.5
      = 29.5 分
```
...
**100 分是当前项目能算出来的最高分**（exhaustion 维度作为风险修饰，100 = 无透支信号，会随透支程度降低）。
...
1. **6 维全亮的股不存在**：要求同时满足 涨停 + 主力大额净流入 + 龙虎榜上榜 + 板块第 1 + 题材第 1 + 无透支 → 现实极少
2. **每个维度内部满分极难**：limit 100 要求"早封 + 大封单 + 2 连板"等多重条件
3. **exhaustion 维度惩罚**：多维度共振的强势股往往有一定透支，exhaustion 分数会打折扣

### 想看更高分有 3 条路

| 思路 | 改动 | 上限提升 | 推荐度 |
|---|---|---|---|
| **A. 改回动态分母** | 但会重新引发"单维霸榜" bug | 单维满分能跑 100 | ❌ 不要 |
| **B. 归一化显示** | 跑完后 `final / 100 × 100` 做视觉缩放 | 24 → 24 | ⭐ 治标 |
| **C. 接受现状** | 看排名而非绝对分数 | 不变 | ⭐⭐⭐⭐ 实用 |
