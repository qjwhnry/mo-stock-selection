...
> 文档生成：2026-04-25（含归一到 100 + 断板反包 + 板块溢出修订）
...
## 3. limit 维度（异动涨停 + 断板反包 + 板块涨停溢出）

### 关键设计：避开 hard_reject 干扰

**当日涨停股会被 `hard_reject.exclude_today_limit_up` 全部踢出 picked**。所以 LimitFilter 的真实产出是两类「跟涨停相关但今天非涨停」的股票（PLAN.md 设计：「**首板 > 连板首日 > 断板反包**」+「**当日涨停股只作为板块信号**」）：
...
**昨天涨停 + 今天没涨停 + 今天保持强势** = 断板反包股。这种股 hard_reject 不过滤。
...
```
final = (0×0.25 + 18×0.25 + 55×0.20 + 45×0.15 + 0×0.15) / 1.00
      = 0 + 4.5 + 11 + 6.75 + 0
      = 22.25 分
```
...
```
理想最强股（4 维全各 100 + sentiment 0）：
  = (100×0.25 + 100×0.25 + 100×0.20 + 100×0.15 + 0×0.15) / 1.00
  = 25 + 25 + 20 + 15 + 0
  = 85 分
```

**85 分是当前项目能算出来的最高分**（sentiment 维度未实现，永远 0，占 0.15 权重）。
...
1. **5 维全亮的股不存在**：要求同时满足 涨停 + 主力大额净流入 + 龙虎榜上榜 + 板块第 1 + 利好情绪 → 现实极少
2. **每个维度内部满分极难**：limit 100 要求"早封 + 大封单 + 2 连板"等多重条件
3. **sentiment 0.15 权重永远 0**：直接让上限降 15 分
...
输出该股 4 维度的详细 detail（各加分项、阈值命中情况），便于复盘"为什么没进 TOP 20"。
