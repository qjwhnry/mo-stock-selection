...
## 7. exhaustion 维度（短线动量质量检测）

**目标**：评估短期动量质量，检测动量衰竭信号（不是入场信号）。
...
当前活跃的 4 个动量质量信号：
1. 冲高回落惩罚（长上影 + 收盘弱 + 近期上涨 + 当天冲高，max=15）
2. 量价背离惩罚（缩量上涨 / 放量滞涨，max=20）
3. 连涨天数惩罚（连续阳线过多，max=10）
4. 动量衰减惩罚（涨幅集中在前半段，max=10）

已禁用的 2 个信号（penalty_max=0）：
5. 5 日累计涨幅（短线追势中不应惩罚涨幅）
6. MA5 偏离度（强势股天然偏离均线）
```
...
| 动量质量信号 | 条件 | 惩罚上限 |
|---|---|---|
| 冲高回落 | 上影线占比 > 60%，收盘位置 < 50%，近 3 日涨幅 > 5%，high 相对昨收涨幅 >= 3% | -15 |
| 缩量上涨 | 近 3 日涨幅 > 5% 且今日量 < 5 日均量×60% | -20 |
| 放量滞涨 | 今日量 > 5 日均量×2 倍且涨幅 < 2% | -16 |
| 连涨天数 | ≥ 7 天 | -10 |
| 连涨天数 | 5-6 天 | -7 |
| 连涨天数 | 3-4 天 | -3.5 |
| 动量衰减 | 5 日涨 > 10% 但近 2 日涨幅占比 < 20% | -10 |
...
| **exhaustion** | 5 日涨幅低、动量健康 → 新鲜度 ≈95 | +0 | **95** |
...
**100 分是当前项目能算出来的最高分**（exhaustion 维度作为动量质量检查，100 = 动量健康）。
