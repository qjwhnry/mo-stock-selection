# Theme And LHB Integration Implementation Plan (v2)
...
| `limit_cpt_list` 积分凭印象写 8000 | 不假定积分；Task 1 增加积分实测步骤，结果回写 memory |
| **题材加分塞进 sector 维度后会触顶（多数股满分 100）** | 新增独立 `theme` 维度，`weights.yaml` 加 `theme: 0.10`，固定分母 1.00 不变 |
| **`Lhb.seat` JSONB 不利于 SQL 查询与历史画像** | 改建独立表 `lhb_seat_detail` (PK=trade_date, ts_code, seat_no)；JSONB 字段在 alembic migration 中 drop |
...
- `src/mo_stock/scheduler/daily_job.py` — 同上
...
`HotMoneyList`（**含 aliases 字段防游资改名**）：
...
class HotMoneyList(Base):
    ...
    aliases: Mapped[dict[str, Any] | None] = mapped_column(
        JSONB, comment="历史名变化记录，格式 {'2024-Q1': '原名 X', ...}",
    )
...
class LhbSeatDetail(Base):
    ...
    trade_date: Mapped[date] = mapped_column(Date, comment="交易日")
    ts_code: Mapped[str] = mapped_column(String(12), comment="股票代码")
    seat_no: Mapped[int] = mapped_column(Integer, comment="席位序号 1-N，按 top_inst 顺序")
    ...
    __table_args__ = (
        PrimaryKeyConstraint("trade_date", "ts_code", "seat_no"),
...
- Memory update: `C:/Users/shimi/.claude/projects/.../memory/tushare_account.md`
...
_UPSERT_BINDINGS_NEW = [
    ...
    (repo.upsert_lhb_seat_detail,       LhbSeatDetail,        ["trade_date", "ts_code", "seat_no"]),
]
...
def upsert_lhb_seat_detail(session, rows):
    return upsert_rows(session, LhbSeatDetail, rows, conflict_cols=["trade_date", "ts_code", "seat_no"])
...
def get_lhb_seats_today(session, trade_date: date) -> dict[str, list[LhbSeatDetail]]:
    ...
    stmt = (
        ...
        .order_by(LhbSeatDetail.ts_code, LhbSeatDetail.seat_no)
    )
...
def _top_inst_rows_from_df(df: pd.DataFrame, hot_money_orgs: set[str]) -> list[dict[str, Any]]:
    ...
    PK = (trade_date, ts_code, seat_no)，seat_no 按同 ts_code 内出现顺序 1..N。
    """
    if df.empty:
        return []
...
class ThemeFilter(FilterBase):
    ...
    def score_all(self, session: Session, trade_date: date) -> list[ScoreResult]:
        ...
        if not top_themes:
            logger.warning("ThemeFilter: {} 无 ths_daily 数据", trade_date)
            return []
...
def _bonus_from_table(table: dict[int, int], rank: int) -> int:
    ...
    策略：找 ≤ rank 的最大 key 对应的分数（而非精确等于）。
...
- Modify: `src/mo_stock/scheduler/daily_job.py`（同上）
...
```bash
# 先备份
docker compose exec pg pg_dump -U mo_stock -d mo_stock > /tmp/backup-pre-theme.sql

# 应用
.venv/Scripts/python.exe -m alembic upgrade head

# 验证
docker compose exec pg psql -U mo_stock -d mo_stock -c "\dt" | grep -E "ths_daily|limit_concept|hot_money|lhb_seat"
```
...
# 3. 检查 6 个维度得分都有
docker compose exec pg psql -U mo_stock -d mo_stock -c \
  "SELECT dim, COUNT(*) FROM filter_score_daily WHERE trade_date='2026-04-24' GROUP BY dim;"
...
- [ ] 现有 254 个测试不破坏（test_moneyflow_filter_scores 这个 pre-existing 失败可暂留）
...
- [ ] `tushare_account.md` memory 更新已实测接口列表
